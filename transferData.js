...
async function exportTable(tableName, targetDb, sqlPool, config) {

    console.log("Replicating " + tableName + " to mongodb");
    const collection = targetDb.collection(tableName);

    const queryCantRegistros = "select count(*) as cantidad from [" + tableName + "]";
    ...
    while (cant >= 0) {
        const query = "select * from [" + tableName + "]" + "order by id_smiafiliados offset " + skip + " rows fetch next " + batch + " rows only";
...
async function main() {
    ...
    await exportTable(config.targetTable, targetDb, sqlPool, config);
}
